...
fn store_erc20_burned_transactions_works() {
	new_test_ext().execute_with(|| {
		assert_ok!(ClaimModule::change_relayer(RawOrigin::Root.into(), 1));
		...
		assert_ok!(ClaimModule::store_erc20_burned_transactions(
			Origin::signed(1),
			10000,
...
fn store_erc20_burned_transactions_no_relayer() {
	new_test_ext().execute_with(|| {
		...
		let eth_addr = EthereumAddress(hex!["9863a44303d29120b00d2d9ea6344751d9ad9ccc"]);
		assert_noop!(
			ClaimModule::store_erc20_burned_transactions(
				Origin::signed(1),
				10000,
